...
- Always use `#problems` to check for compile/lint errors the Implementer may have missed.
...
- Use `#usages` to verify that changes to shared code don't break other consumers.
...
4. **Use tools** — run `#problems` for compiler/linter errors, `#usages` for dependency checks, `#search` for pattern verification.
...
- [ ] No `#problems` errors (compile/lint) after implementation.
